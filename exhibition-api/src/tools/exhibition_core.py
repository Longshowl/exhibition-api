...
def search_exhibition_info_core(exhibition_name: str) -> str:
    """
    ...
    """
    ctx = new_context(method="search.exhibition")
...
def search_exhibitor_list_core(exhibition_name: str) -> str:
    """
    ...
    Returns:
        包含参展商名单、企业列表等信息的字符串
    """
    ctx = new_context(method="search.exhibitor")
    client = SearchClient(ctx=ctx)
...
def search_exhibition_by_keywords_core(
    keywords: str,
    ...
    Returns:
        包含相关展会列表和链接的字符串
    """
    ctx = new_context(method="search.exhibition.keywords")
    client = SearchClient(ctx=ctx)
